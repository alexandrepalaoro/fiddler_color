...
# Crabs have a dichromatic vision with peaks on the blue and green spectrum, so we're analyzing 
# the combination of those spectra. Given that the residuals are poorly behaved for most the correlations, 
# we are using the Spearman's rank test to test the correlation between brightness 1 and all quality indices. 
...
# Now we are shifting to another aspect of color: hue. To get the hue for dichromatic vision, 
# we divided raw blue by raw green reflectance and reran the correlation tests we did before. 
...
# We are using a secondary measure of hue because, while crabs are largely insensitive to red wavelengths, 
# some manifestations of the red can be seen for the crabs. Instead of seeing red, they might see the claw
# as black, for instance. Thus, we opted to do a second measure of hue that includes red. To do this, we 
# divided blue by the sum of green and red reflectance. We then ran ran the same analysis shown above.
...
# To do this, we used a beta regression (i.e., a generalized linear model with a beta distribution) 
# because reflectance values are constrained between 0 and 1, which fit this distribution while not 
